...
num_heads = 2
num_epochs = 10000
checkpoint_directory = "attention_island/"
...
class IslandsOfVectors(nn.Module):
    def __init__(self, d_model, iterations=3, tau=0.5):
        super(IslandsOfVectors, self).__init__()
    ...
    def forward(self, vectors):
        for _ in range(self.iterations):
            similarities = torch.matmul(vectors, vectors.transpose(-2, -1))
            attention_weights = F.softmax(similarities / self.tau, dim=-1)
            vectors = torch.matmul(attention_weights, vectors)
        return vectors
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model):
        ...
        self.islands_of_vectors = IslandsOfVectors(d_model).to(device)
        self.layer = NetworkLayer(d_model)

    def forward(self, x):
        
        # Apply attention islands
        x = self.islands_of_vectors(x)
